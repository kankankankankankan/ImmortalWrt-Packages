...
var updatePanelId = 'fakehttp-update-panel';
var updateHelper = '/usr/libexec/fakehttp-update';
...
function getCurrentVersionText(updateInfo) {
	var fakehttpPkg = getPackageInfo(updateInfo, 'fakehttp');
	var luciPkg = getPackageInfo(updateInfo, 'luci');
	var fakehttpVersion = formatPackageVersion(fakehttpPkg);
	var luciVersion = formatPackageVersion(luciPkg);

	if (fakehttpVersion === luciVersion)
		return fakehttpVersion;

	return '服务 ' + fakehttpVersion + ' / LuCI ' + luciVersion;
}
...
function renderReleaseValue(latest) {
	...
	return E('span', { 'class': 'fakehttp-mono' }, [ tag ]);
}

function showFakehttpModal(title, children) {
...
function confirmInstallUpdate(updateInfo) {
	...
	return new Promise(function(resolve) {
		showFakehttpModal('确认在线更新', [
			E('div', { 'class': 'fakehttp-update-confirm' }, [
				E('p', {}, [
					'将从 GitHub Release 下载与当前系统完全匹配的 FakeHTTP 安装包，并调用系统包管理器安装至 ',
					E('strong', {}, [ version ]),
					'。'
				]),
				E('p', {}, [
		...
		], 'cbi-modal', 'fakehttp-update-confirm-modal');
	});
}
...
function notifyUpdateResult(data, command) {
	...
	if (data.log)
		body.push(E('pre', { 'class': 'fakehttp-notification-log' }, [
...
function renderUpdateProgress(progress) {
	...
	return E('div', { 'class': 'fakehttp-update-progress' }, [
		E('div', { 'class': 'fakehttp-update-progress-head' }, [
			E('span', { 'class': labelClass }, [ getProgressStageText(stage) ]),
			message ? E('span', { 'class': 'fakehttp-update-status-text' }, [ message ]) : null
		].filter(function(el) { return el != null; })),
		log ? E('pre', { 'class': 'fakehttp-update-log' }, [ tailTextNewestFirst(log, 120, '暂无升级日志') ]) : null
	].filter(function(el) { return el != null; }));
}
...
function renderUpdatePanel(viewObj) {
	...
	return E('div', { 'id': updatePanelId, 'class': 'fakehttp-update-panel' }, [
		E('div', { 'class': 'fakehttp-update-brief-item' }, [
			E('span', { 'class': 'fakehttp-update-brief-label' }, [ '当前版本' ]),
			E('span', { 'class': 'fakehttp-mono' }, [ getCurrentVersionText(updateInfo) ])
		]),
		E('div', { 'class': 'fakehttp-update-brief-item' }, [
			E('span', { 'class': 'fakehttp-update-brief-label' }, [ '最新版本' ]),
			E('span', { 'class': 'fakehttp-mono' }, [ latestVersion ])
		]),
		E('div', { 'class': 'cbi-value-description fakehttp-update-meta' }, metaChildren),
		E('div', { 'class': 'fakehttp-action-group fakehttp-update-actions' }, [
			E('button', btnProps, [ btnText ]),
			msg ? E('span', { 'class': 'fakehttp-update-status-text' }, [ msg ]) : null
		].filter(function(el) { return el != null; })),
